...
cfg = RetrainerConfig("./liteml_config.yaml", custom_tracer= CustomTracer)
data_bits = cfg['QAT']['data-quantization']['bits']
w_bits = cfg['QAT']['weights-quantization']['bits']
...
args.model = torch.nn.LSTM

task = Task.init(project_name='Regression tests/lstm' ,
                 task_name="{} retraining".format("lstm" if args.model == torch.nn.LSTM else 'gru'),
                 tags=["{}/{}".format(data_bits, w_bits)])
if not os.path.exists("tensorboard"):
    os.makedirs("tensorboard")
writer = SummaryWriter(log_dir= "tensorboard")


if args.model == torch.nn.LSTM:
    args.load_weights_from = "checkpoints_3layerbidirectional_lstm/002.ckpt"
elif args.model == torch.nn.GRU:
    args.load_weights_from = "checkpoints_3layerbidirectional_gru/025.ckpt"
else:
    raise Exception(f"unsupported rnn type: {args.model}")
...
# Data loader
train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=30)
eval_loader = torch.utils.data.DataLoader(dataset=eval_dataset, batch_size=args.batch_size, shuffle=False, num_workers=30)
calibration_loader = torch.utils.data.DataLoader(dataset=calibration_dataset, batch_size=args.batch_size, shuffle=False, num_workers=30)


sample, classes = next(iter(calibration_loader))
sample = sample.cuda()
# Create model and train -------------------------------------------------
model = lib_rnn.create_RNN_model(args, load_weights_from=args.load_weights_from, bidirectional=True)# #
...
if wrap_in_retrainer:
    cfg.optimizations_config["QAT"]['calibration_loader'] = calibration_loader
    cfg.optimizations_config["QAT"]['calibration_loader_key'] = lambda model, x:model(x[0].cuda())
